...
| `just test-local-source`| You changed `_scan_local_source`, the `local`-linking functions, or `_cleanup_stale_for_tool`'s no-delete guarantee for `local`'s canonical store (`.harness-ai/skills/local/`/`.harness-ai/agents/local/`, design.md D2/D3 of `harness-ai-local-canonical-store`). |
| `just test-local-source-migration-safety` | You changed `_make_symlink`'s foreign-content guard as it applies to an unmigrated pre-1.0 `local` workspace (real file still at `.agents/skills/<key>`) — confirms the guard blocks the migration symlink instead of clobbering it (design.md D4). |
