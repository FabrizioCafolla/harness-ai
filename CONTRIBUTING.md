...
| `just test-symlinks`    | You changed the canonical-store render step or the tool-dir symlink logic (`_render_skill`/`_render_agent`/`_make_symlink`) for `harness-ai`/`contentRepos` sources. |
| `just test-local-source`| You changed `_scan_local_source`, the `local`-linking functions, or `_cleanup_stale_for_tool`'s no-delete guarantee for `local`'s canonical store (`.harness-ai/skills/local/`/`.harness-ai/agents/local/`). |
| `just test-local-source-migration-safety` | You changed `_make_symlink`'s foreign-content guard as it applies to an unmigrated pre-1.0 `local` workspace (real file still at `.agents/skills/<key>`) — confirms the guard blocks the migration symlink instead of clobbering it. |
| `just test-category-filter` | You changed `_apply_skill_filter` or the `skills.include`/`skills.exclude` config/CLI plumbing. |
| `just test-workspace-source` | You changed the auto-detected `workspace` source (`.harness-ai/local/`) — detection, load-order precedence against `harness-ai`/`contentRepos`, or its interaction with `local`'s absolute precedence. |
| `just test-foreign-entry` | You changed `_make_symlink`'s foreign-content guard, any of its four call sites, the end-of-run "unmanaged" sweep, or `_cleanup_stale_for_tool`'s call-site ordering (must run before the `local`-linking pass). |
...
| `just update-skills`    | You're refreshing a bundled skill that tracks an upstream `ref:` (none does today) — not a general-purpose recipe to run on every change. |
