...
# Verify harness-ai/content-repo skills render into the canonical store and
# .claude/.opencode/.agents are symlinks into it (not independent copies),
# and that .agents renders even though 'agents' isn't in --tools.
test-symlinks: clean
    @echo "==> Running scaffold (tools: claude)..."
    mkdir -p {{workspace}}
    {{uv}} \
    ...
    test -L {{workspace}}/.claude/skills/wikictl/SKILL.md \
        && echo "  [OK] .claude/skills/wikictl/SKILL.md is a symlink" \
        || { echo "  [FAIL] .claude/skills/wikictl/SKILL.md is not a symlink"; exit 1; }
    readlink {{workspace}}/.claude/skills/wikictl/SKILL.md | grep -q "harness-ai/skills/harness-ai/wikictl/claude.SKILL.md" \
        && echo "  [OK] symlink resolves into .harness-ai/skills/harness-ai/wikictl/" \
        || { echo "  [FAIL] symlink does not point into the canonical store"; exit 1; }
    test -L {{workspace}}/.agents/skills/wikictl/SKILL.md \
        && echo "  [OK] .agents/skills/wikictl/SKILL.md exists as a symlink (agents wasn't in --tools)" \
        || { echo "  [FAIL] .agents/skills/wikictl/SKILL.md missing or not a symlink"; exit 1; }
...
# `.agents` included, never rendered/copied. Regression-tests the idempotent
...
test-local-source: clean
    ...
    # `.agents` is a pure render target for `local` too, same as every other
    # tool — no same-path exception where `.agents/skills` would be the
    # source instead of a link target.
    ...
    # Regression: the per-tool cleanup call runs before the local-linking
    # pass, and must see a preview of what that pass is about to claim —
    # otherwise every still-valid local key is misdiagnosed as stale and
    # unlinked+relinked on every single run. Harmless to final state (local's
    # own pass immediately recreates the symlink) but wasteful churn and a
    # misleading "removed" count in the sync report, forever, on every run,
    # for every local skill — not caught by the seen/linked-only regex
...
# restrict to one category, then carve one key out of it.
# The reverse migration: a key leaving `local` and going back to a
# harness-ai/repo source. `local`'s tool-dir shape is a whole-directory
# symlink, and the cleanup that removes it runs after the render, so the
# render has to cope with finding it there. This crashed with
# FileExistsError on the dangling link.
test-local-source-migration-out: clean
    @echo "==> First run: 'local' claims a key that 'harness-ai' also ships..."
    mkdir -p {{workspace}}/.harness-ai/skills/local/wikictl
    printf -- '---\nname: wikictl\ndescription: Local override.\n---\n\nLocal body.\n' \
        > {{workspace}}/.harness-ai/skills/local/wikictl/SKILL.md
    {{uv}} --workspace {{workspace}} --tools claude --install-defaults true \
        --create-file-mcp false --create-file-hooks false --create-file-setting false \
        --update-gitignore false | tee {{workspace}}-run1.log
    test -L {{workspace}}/.claude/skills/wikictl \
    ...
    @echo "==> Removing the local source: the key must fall back to 'harness-ai', not crash..."
    rm -rf {{workspace}}/.harness-ai/skills/local/wikictl
    rm -f {{workspace}}/.harness-ai/lock
    {{uv}} --workspace {{workspace}} --tools claude --install-defaults true \
        --create-file-mcp false --create-file-hooks false --create-file-setting false \
        --update-gitignore false | tee {{workspace}}-run2.log
    test ! -L {{workspace}}/.claude/skills/wikictl \
        && echo "  [OK] tool dir is a real directory again (harness-ai's shape)" \
        || { echo "  [FAIL] still local's directory-symlink shape"; exit 1; }
    test -L {{workspace}}/.claude/skills/wikictl/SKILL.md \
        && echo "  [OK] SKILL.md symlinked from the harness-ai canonical store" \
        || { echo "  [FAIL] SKILL.md not linked after the migration out of local"; exit 1; }
    grep -q "Local body" {{workspace}}/.claude/skills/wikictl/SKILL.md \
        && { echo "  [FAIL] CRITICAL: still serving the removed local body"; exit 1; } \
        || echo "  [OK] content comes from harness-ai, not the removed local copy"
...
test-skill-paths: clean
    ...
    grep -q 'disable-model-invocation: true' {{workspace}}/.harness-ai/skills/frompaths/solo/claude.SKILL.md \
        && echo "  [OK] upstream frontmatter preserved verbatim, extra keys included" \
        || { echo "  [FAIL] upstream frontmatter keys lost"; exit 1; }
    grep -q 'license' {{workspace}}/.harness-ai/skills/frompaths/solo/claude.SKILL.md \
    ...
    grep -qE 'frompaths +skill +2 +2' {{workspace}}-run1.log \
        && echo "  [OK] sync summary counts the frompaths source" \
        || { echo "  [FAIL] frompaths missing from the sync summary"; exit 1; }
    ...
    mkdir -p {{workspace}}
    {{uv}} --workspace {{workspace}} --tools claude --install-defaults true \
        --skill-path "solo={{workspace}}-fetch/solo" --from-paths-sha "sha-one" \
        --create-file-mcp false --create-file-hooks false --create-file-setting false \
        --update-gitignore false > /dev/null
    {{uv}} --workspace {{workspace}} --check-only --from-paths-sha "sha-one" > /dev/null \
    ...
    {{uv}} --workspace {{workspace}} --check-only --from-paths-sha "sha-two" > /dev/null \
...
# Category/subcategory/key include+exclude filtering (skills only) needs
# more than one category to prove anything, so it drives its own throwaway
# content repo with two skills in different categories rather than relying
# on whatever happens to be bundled.
test-category-filter: clean
    @echo "==> Content repo with two skills in different categories..."
    mkdir -p {{workspace}} {{workspace}}-repo/skills/meta-a {{workspace}}-repo/skills/meta-b {{workspace}}-repo/skills/other-c
    printf 'skills:\n  meta-a:\n    category: meta\n    subcategory: creation\n    claude:\n      name: meta-a\n      description: A.\n  meta-b:\n    category: meta\n    subcategory: creation\n    claude:\n      name: meta-b\n      description: B.\n  other-c:\n    category: engineering\n    subcategory: coding\n    claude:\n      name: other-c\n      description: C.\n' \
        > {{workspace}}-repo/skills/metadata.yml
    printf 'A body.\n' > {{workspace}}-repo/skills/meta-a/SKILL.md
    printf 'B body.\n' > {{workspace}}-repo/skills/meta-b/SKILL.md
    printf 'C body.\n' > {{workspace}}-repo/skills/other-c/SKILL.md
    @echo "==> Running scaffold (skills.include.categories=meta, exclude.keys=meta-b)..."
    {{uv}} \
        --workspace {{workspace}} \
        --tools claude \
        --create-file-mcp false \
        ...
        --update-gitignore false \
        --install-defaults true \
        --content-repos "filtertest={{workspace}}-repo" \
        --skills-include-categories meta \
        --skills-exclude-keys meta-b
    @echo "==> Verifying filter..."
    test -d {{workspace}}/.claude/skills/meta-a \
        && echo "  [OK] meta-a present (category: meta, included)" \
        || { echo "  [FAIL] meta-a missing"; exit 1; }
    test ! -e {{workspace}}/.claude/skills/meta-b \
        && echo "  [OK] meta-b absent (excluded by key from an otherwise-included category)" \
        || { echo "  [FAIL] meta-b present, should have been excluded"; exit 1; }
    test ! -e {{workspace}}/.claude/skills/other-c \
        && echo "  [OK] other-c absent (category: engineering, not included)" \
        || { echo "  [FAIL] other-c present, should have been filtered out by category"; exit 1; }
...
# No bundled command ships today, so this drives its own throwaway content repo.
test-commands: clean
    @echo "==> First run: command from a content repo..."
    mkdir -p {{workspace}}-repo/commands/dev
    printf 'commands:\n  dev/deep-task-analysis:\n    claude:\n      name: deep-task-analysis\n      description: Test command.\n      argument-hint: "[task]"\n' \
        > {{workspace}}-repo/commands/metadata.yml
    printf 'Command body.\n' > {{workspace}}-repo/commands/dev/deep-task-analysis.md
    {{uv}} \
        --workspace {{workspace}} \
        --tools claude \
        --create-file-mcp false \
        ...
        --update-gitignore false \
        --install-defaults true \
        --content-repos "cmdtest={{workspace}}-repo" \
        | tee {{workspace}}-run1.log
    ...
    readlink {{workspace}}/.claude/commands/dev/deep-task-analysis.md \
        | grep -q '.harness-ai/commands/cmdtest/dev/deep-task-analysis/claude.md' \
        && echo "  [OK] symlink points into the canonical store" \
        || { echo "  [FAIL] symlink target unexpected"; exit 1; }
    grep -q 'argument-hint' {{workspace}}/.harness-ai/commands/cmdtest/dev/deep-task-analysis/claude.md \
    ...
    grep -q "command 'dev/deep-task-analysis' claimed by local" {{workspace}}-run2.log \
        && echo "  [OK] repo render skipped in favour of local" \
    ...
    readlink {{workspace}}/.claude/commands/dev/deep-task-analysis.md \
        ...
        || { echo "  [FAIL] symlink still points at the repo source"; exit 1; }
    ...
    readlink {{workspace}}/.claude/commands/dev/deep-task-analysis.md \
        | grep -q '.harness-ai/commands/cmdtest/dev/deep-task-analysis/claude.md' \
        && echo "  [OK] key fell back to the content repo source" \
        || { echo "  [FAIL] key did not fall back to the repo source after local was removed"; exit 1; }
    rm -rf {{workspace}}-repo
    @echo "test-commands: OK"

test-workspace-source: clean
    @echo "==> Seeding .harness-ai/local/ overriding the bundled 'wikictl' skill..."
    mkdir -p {{workspace}}/.harness-ai/local/skills/wikictl {{workspace}}/.harness-ai/local/agents
    printf 'default:\n  claude:\n  opencode:\n\nagents:\n' \
        > {{workspace}}/.harness-ai/local/agents/metadata.yml
    printf 'default:\n  claude:\n  opencode:\n\nskills:\n  wikictl:\n    category: meta\n    subcategory: creation\n    claude:\n      name: wikictl\n      description: Workspace override of wikictl.\n    opencode:\n      name: wikictl\n      description: Workspace override of wikictl.\n' \
        > {{workspace}}/.harness-ai/local/skills/metadata.yml
    printf '# Wikictl (workspace override)\nWORKSPACE-OVERRIDE-BODY-MARKER\n' \
        > {{workspace}}/.harness-ai/local/skills/wikictl/SKILL.md
    ...
    readlink {{workspace}}/.claude/skills/wikictl/SKILL.md | grep -q "harness-ai/skills/workspace/wikictl" \
        && echo "  [OK] wikictl symlinks into the workspace source's canonical store (won over harness-ai)" \
        || { echo "  [FAIL] wikictl did not resolve to the workspace source"; exit 1; }
    grep -q "WORKSPACE-OVERRIDE-BODY-MARKER" {{workspace}}/.harness-ai/skills/workspace/wikictl/claude.SKILL.md \
    ...
    # Regression test: .harness-ai/local/ is a plain
    ...
    {{uv}} \
        ...
        --update-gitignore false \
        --install-defaults true
    grep -q "WORKSPACE-OVERRIDE-BODY-MARKER-V2" {{workspace}}/.harness-ai/skills/workspace/wikictl/claude.SKILL.md \
    ...
    # No need to touch .agents/skills/wikictl by hand first: it's
    ...
    # source migration.
    mkdir -p {{workspace}}/.harness-ai/skills/local/wikictl
    printf -- '---\nname: wikictl\ndescription: Local override of wikictl.\n---\n\nLOCAL-OVERRIDE-BODY-MARKER\n' \
        > {{workspace}}/.harness-ai/skills/local/wikictl/SKILL.md
    rm -f {{workspace}}/.harness-ai/lock
    {{uv}} \
        --workspace {{workspace}} \
        ...
        --install-defaults true \
        | tee {{workspace}}-run2.log
    grep -q "\[collision\] skill 'wikictl' claimed by local — skipping render from 'workspace'" {{workspace}}-run2.log \
        && echo "  [OK] collision report: local skipped workspace's render, as designed" \
        || { echo "  [FAIL] expected collision line not found"; exit 1; }
    readlink {{workspace}}/.claude/skills/wikictl | grep -q '.harness-ai/skills/local/wikictl' \
        && echo "  [OK] wikictl now symlinks to the real local source (local beats workspace)" \
        || { echo "  [FAIL] wikictl did not resolve to the local source"; exit 1; }
    readlink {{workspace}}/.agents/skills/wikictl | grep -q '.harness-ai/skills/local/wikictl' \
        && echo "  [OK] .agents/skills/wikictl also re-pointed to the local source (agents profile no longer exempt)" \
        || { echo "  [FAIL] .agents/skills/wikictl did not resolve to the local source"; exit 1; }
    rm -f {{workspace}}-run1.log {{workspace}}-run2.log

# Foreign-entry safety: a real, non-symlink file harness-ai
...
test-foreign-entry: clean
    @echo "==> Pre-seeding a foreign (hand-placed) SKILL.md at a key 'harness-ai' also defines..."
    mkdir -p {{workspace}}/.claude/skills/wikictl {{workspace}}/.claude/skills/my-scratch-skill
    printf '# Not harness-ai\nFOREIGN-BODY-MARKER\n' > {{workspace}}/.claude/skills/wikictl/SKILL.md
    ...
    {{uv}} \
        ...
        --install-defaults true \
        | tee {{workspace}}-run1.log
    grep -q "FOREIGN-BODY-MARKER" {{workspace}}/.claude/skills/wikictl/SKILL.md \
        && echo "  [OK] foreign SKILL.md left byte-for-byte untouched" \
        || { echo "  [FAIL] CRITICAL: foreign content was overwritten"; exit 1; }
    test ! -L {{workspace}}/.claude/skills/wikictl/SKILL.md \
        && echo "  [OK] wikictl/SKILL.md is still a real file, not a symlink" \
        || { echo "  [FAIL] wikictl/SKILL.md became a symlink"; exit 1; }
    grep -q "\[foreign\] skill 'wikictl'" {{workspace}}-run1.log \
    ...
    grep -qE 'claude +harness-ai.*[1-9][0-9]*$' {{workspace}}-run1.log \
        && echo "  [OK] summary table's foreign column is nonzero for (claude, harness-ai)" \
        || { echo "  [FAIL] summary table foreign column did not reflect the blocked render"; exit 1; }
    grep -q "unmanaged entries" {{workspace}}-run1.log \
        && echo "  [OK] unmanaged section printed" \
        || { echo "  [FAIL] unmanaged section missing"; exit 1; }
    grep -q ".claude/skills/wikictl" {{workspace}}-run1.log \
